...
<xsl:template match="contestStandings">
    <HTML>
        <HEAD>
<TITLE>
Summary of 7th LCPC - <xsl:value-of select="/contestStandings/standingsHeader/@title"/>
</TITLE>
...
<style>
        th { background: #0676bd; color: white; height:30px;padding:0 10px 0 10px; }
        tr:nth-child(odd) { background: #a2c042; color: white; }
...
<p>
Installed and configured by ACPC team
 <A HREF="http://www.ecs.csus.edu/pc2">CSUS PC^2 <xsl:value-of select="/contestStandings/standingsHeader/@systemVersion"/></A><br/>
...
<td align="center"><xsl:value-of select="@totalAttempts"/>/<xsl:value-of select="@totalSolved"/></td>
                </tr>
            </xsl:for-each>
        </xsl:template>
...
<td align="center"><xsl:value-of select="@points"/></td>
...
<td align="center"><xsl:value-of select="@totalAttempts"/>/<xsl:value-of select="@solved"/></td>
                </tr>
            </xsl:for-each>
        </xsl:template>
